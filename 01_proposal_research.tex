...
\begin{figure}[!hbt]
    ...
    \caption{A fine-scale topographic map of the Lutz~catchment on Barro~Colorado~Island, Panama, showing the locations of the weir, tower, and ten soil moisture sampling sites.\autocite{stri2025}}
    % \caption{The location of Barro~Colorado~Island in Panama CITE.}
    \label{fig_map}
\end{figure}

Run-off data has been electronically collected from the weir since 1989 in frequencies ranging from three~minutes to three~hours, resulting in millions of data points with more added every day.
...
Currently, quality assurance is conducted manually in Visual~FoxPro.
...
% 
Rainfall run-off data collected by citizen scientists in Ethiopia has been visually (via graphical inspections) and quantitatively (via statistical methods) analyzed for quality compared to nearby professional references.\autocite{mengistiea2024}
...
Furthermore, it is not currently set up to automatically detect problematic portions of the data.
Additionally, because data monitoring is ongoing, new data must be regularly checked for occurrence of any type of failure mode.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{rllr}
        ...
        \midrule
        \secrefboth{sec_es} & Gillian McGinnis & 323 \\
        \secrefboth{sec_lit} & \ditto & 308 \\
        \secrefboth{sec_deliv} & \ditto & 387 \\
